  ...
  <div>
    ...
    >
      ...
      <flex-col
        ...
        xs="12"
        xl="9"
        class="max-allowed-container--sidebar overflow-y-auto thinScrollbar"
      >
        <div
          class="content"
        >
    ...
    <div
      class="navigation-bar p-10"
    >
      <div
        class="flex-row justify-around"
      >
        <flex-col
          v-for="navbarAction in navbarActions"
          :key="navbarAction['name']"
          col="2"
          class="navbar-action-container"
        >
          <img :src="require('@assets/media/svg/'+navbarAction['icon']+'.svg')" >
          <div>
            <span
              :class="navbarAction['labelClass']"
            >
              {{ navbarAction['label'] }}
            </span>
          </div>
...
export default {
  ...
  setup () {
    ...
    function buildSampleData () {
      ...
      let cards = [];
      [...Array(getRandomNumber(3, 10)).keys()].forEach(ind => {
    ...
    function buildSampleCard () {
      ...
      return {
        ...
        transactions: [...Array(getRandomNumber(1, 20)).keys()].map(transactionInd =>
          getRandomTransaction(card['uuid']))
      };
    }
