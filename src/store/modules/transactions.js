...
const getters = {
  ...
  getTransactionsByCard: (state) => (card) => state['transactions'].filter(transaction => transaction['card'] === card)
};
...
const mutations = {
  ...
  REMOVE_TRANSACTION: function (state, transaction) {
    let transactionIndex = state['transactions'].findIndex(st => st['id'] === transaction['id']);
